...
class IEGMNetLSTM(nn.Module):
    def __init__(self):
        ...
        self.lstm = nn.LSTM(input_size=20, hidden_size=20, batch_first=True)
    ...
    def forward(self, input):
        ...
        conv5_output = conv5_output.reshape(-1, 36, 20)

        lstm_output, _ = self.lstm(conv5_output)
        lstm_output = lstm_output[:,-1,:]

        # flat_output = self.flat(conv5_output)

        # fc1_output = self.fc1(flat_output)
        fc2_output = self.fc2(lstm_output)
        return fc2_output
